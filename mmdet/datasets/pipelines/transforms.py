...
class Albu(object):
    ...
    def __init__(self,
        ...
        self.keypoint_params = (
            self.albu_builder(keypoint_params) if keypoint_params else None)
    ...
    def __call__(self, results):
        ...
        if 'keypoints' in results:
            ...
            if isinstance(results['keypoints'], np.ndarray):
                results['keypoints'] = results['keypoints'].reshape(-1, 3).tolist()
            # add pseudo-field for filtration
            # if self.filter_lost_elements:
            #     results['idx_mapper'] = np.arange(len(results['keypoints']))
        # numb_kps = len(results['keypoints'])
        ...
        # while len(results['keypoints'])!=numb_kps:
        #     results = self.aug(**results)
        ...
        if 'keypoints' in results:
            if isinstance(results['keypoints'], list):
                results['keypoints'] = np.array(
                    results['keypoints'], dtype=np.float32)
            results['keypoints'] = results['keypoints'].reshape(-1, 5, 3)
