...
class AnchorGenerator(object):
    ...
    def grid_anchors(self, featmap_sizes, device='cuda'):
        ...
        """
        # print(featmap_sizes) # [80, 80], [40, 40], [20, 20], [10, 10], [5, 5]
        ...
        #     debug = anchors.clone()
        #     # debug = torch.cat(debug, dim=0)
        #     dets = debug.detach().numpy()
        #     dets[2:4] = dets[2:4]/featmap_sizes[i][0]*640
        #     dets = dets[2*(dets[:,3]-dets[:,1])<dets[:,2]-dets[:,0]]
        #     for i, b in enumerate(dets.tolist()):
        #         b = list(map(int, b))
        #         color = np.random.randint(255, size=3)
        #         cv2.rectangle(img, (b[0], b[1]), (b[2], b[3]), (int(color[0]), int(color[1]), int(color[2])), 2)
        # cv2.imwrite('/ssd_keras/scrfd/demo/debug.jpg', img)
